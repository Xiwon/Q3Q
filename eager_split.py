"""
Q3Q :: Qwen3.5 MoE Text-Only Inference (Eager, prefill/decode split)
...
class QMoeEngine(nn.Module):
    ...
    @torch.no_grad()
    def decode_step(self, next_token: torch.Tensor, state: dict):
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            ...
            if eos_set and next_tok.item() in eos_set:
                break
            logits, state = self.decode_step(next_tok, state)
        return result
